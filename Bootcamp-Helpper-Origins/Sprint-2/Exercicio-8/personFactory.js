...
export default class PersonFactory {
    ...
    getPerson(list) {
        switch (list.id) {
            case 1:
                let ada = new AdaLovelace(list.id, list.name, list.bio);
                (list.creatorAda == 'sim') ? ada.showList(ada.creatorAda()) : ada.showList(ada.getArray());
                break;
            case 2:
                let alan = new AlanTuring(list.id, list.name, list.bio);
                alan.showList(alan.getArray());
                break;
            case 3:
                let nikola = new NikolaTesla(list.id, list.name, list.bio);
                nikola.showList(nikola.getArray());
                break;
            case 4:
                let nicolau = new NicolauCopernico(list.id, list.name, list.bio);
